...
function PlaylistView() {
    ...
    const handleDeleteGebaar = (gebaarId) => {
        const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2N2RhYjAxNTVmN2I5NzgyMjU1MDg0MTkiLCJyb2xlIjoiYWRtaW4iLCJpYXQiOjE3NDIzODUxNzMsImV4cCI6MTc0MjQwMzE3M30.xZWGo1-_PRWmbOleHMeWu0trg3V4-6rrFVttSJXrZJY";

        fetch(`http://145.24.223.113:8000/playlist/${id}`, {
            method: "DELETE",  // Gebruik DELETE om een gebaar te verwijderen
            headers: {
                "Accept": "application/json",
                "Authorization": `Bearer ${token}`,
            ...
            .then(response => {
                ...
                // Verwijder het gebaar uit de state na succesvolle verwijdering
                const updatedGebaren = playlist.gebaren.filter(gebaar => gebaar.id !== gebaarId);
                setPlaylist(prevPlaylist => ({
                    ...prevPlaylist,
                    gebaren: updatedGebaren
                }));
            })
            .catch(error => console.error("Error deleting gebaar:", error));
    };
        ...
        <div className="container mx-auto pl-4">
            <div className="flex items-center mb-[2vw]">
                ...
                <button
                    ...
                    className='flex justify-center items-center w-[3.5vw] h-[3.5vw] rounded-full bg-green-400 ml-[1.5vw] hover:bg-green-700'
                >
            ...
            {playlist.gebaren.length > 0 ? (
                ...
                <p className="text-center text-gray-500">Er zijn geen gebaren meer in deze playlist.</p>
            )}
        </div>
    );
}
